...
const login = async () => {
    ...
    const token = 'Basic ' + Buffer
        ...
        .toString('base64');
    return fetch_1.default(utils_1.getURL.LOGIN(), {
        method: 'POST',
        body: JSON.stringify({ rememberMe: true })
    })(token)
        .then(async (res) => {
        if (res && res.ticket && res.expiration) {
            await fs_1.promises.writeFile(exports.getAuthFilePath(), JSON.stringify(res));
            return res;
        }
        else
...
const setNextLogin = async (auth) => {
    clearTimeout(LOGIN_TIMEOUT);
    LOGIN_TIMEOUT = setTimeout(() => exports.login(), getExpiration(auth));
};
const getAuth = () => exports.login();
exports.getAuth = getAuth;
const getTicket = () => exports.login().then(auth => auth.ticket);
exports.getTicket = getTicket;
const getToken = () => exports.login().then(auth => `Ubi_v1 t=${auth.ticket}`);
...
const getAuthFilePath = () => authFilePath || path_1.join(authFileDirPath, authFileName);
