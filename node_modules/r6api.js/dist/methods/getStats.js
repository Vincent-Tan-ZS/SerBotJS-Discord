...
const getOperatorIconURL = (name) => `${constants_1.GITHUB_ASSETS_URL}/operators/${name.includes('recruit') ? 'recruit' : name}.png`;
const statGetter = (obj, first, second, type) => obj[`${first}${type || ''}_${second}:infinite`] || 0;
...
const operatorsGetter = (obj, type) => Object.entries(constants_1.OPERATORS).reduce((acc, [codeName, cur]) => {
    acc[codeName] = {
        name: cur.name,
        role: cur.role,
        unit: cur.unit,
        ...
        uniqueAbility: cur.uniqueAbility
            ? {
                ...
                icon: (0, utils_1.getCDNURL)(cur.uniqueAbility.iconId),
...
const weaponsGetter = (obj, type) => Object.entries(constants_1.WEAPONTYPES).reduce((acc, [weaponTypeIndex, weaponTypeVal]) => {
    ...
    acc[weaponTypeVal.id].list = Object.entries(constants_1.WEAPONS)
        ...
        acc2[id] = {
            name: weapon.name,
            icon: (0, utils_1.getCDNURL)(weapon.iconId),
...
exports.default = (platform, ids, options) => {
    ...
    return Promise.all(stats.map(chunk => (0, auth_1.getToken)()
        .then((0, fetch_1.default)(utils_1.getURL.STATS(platform, ids, chunk)))))
        .then(res => Object.entries(res
        .reduce((acc, { results: cur }) => {
        Object.keys(cur).map(key => acc[key] = Object.assign(acc[key] || {}, cur[key]));
        return acc;
    }, {}))
        .map(([id, vals]) => ({
        id: id,
